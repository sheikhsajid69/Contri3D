...
// Nike-aware color tokens
const LIGHT_THEMES = new Set(['nike']);
const STREAK_FIRE = '#d30005';
...
export function renderSVG(days, maxTotal, options = {}) {
  ...
  const themeName = options.theme || 'green';
  ...
  const labelColor = isLight ? '#111111' : '#8b949e';
  const mutedColor = isLight ? '#707072' : '#484f58';
  const fontStack = isLight ? "'Inter', system-ui, sans-serif" : 'monospace';
  ...
  <rect width="${viewW}" height="${130 + labelExtra}" fill="${bgColor}" rx="12"/>
  <text x="${viewW / 2}" y="60" text-anchor="middle" fill="${mutedColor}" font-family="${fontStack}" font-size="13">No contribution data available</text>
</svg>`;
  }
  ...
  if (showLabels) {
    ...
    const streakColor = streak > 0 ? STREAK_FIRE : labelColor;

    if (isLight) {
      // Nike editorial label layout — uppercase, Inter font, streak fire accent
      const totalGH = days.reduce((s, d) => s + (d.github || 0), 0);
      labels = `
  <g transform="translate(${centerX - 160}, ${labelY})">
    <text font-family="${fontStack}" font-size="14px" font-weight="500" fill="${labelColor}">↗ ${totalGH.toLocaleString()} CONTRIBUTIONS</text>
    <text x="240" font-family="${fontStack}" font-size="14px" font-weight="500" fill="${streakColor}">🔥 ${streak}-DAY STREAK</text>
  </g>`;
    } else {
      // Classic dark label layout
  ...
  <defs>
    <style>
      .label { fill: ${labelColor}; font-family: ${fontStack}; font-size: 11px; text-anchor: middle; }${animateStyles}
    </style>
  </defs>
  <rect x="${viewX}" y="${viewY}" width="${viewWCalc}" height="${viewHCalc}" fill="${bgColor}" rx="12"/>
  ${bars}${labels}
</svg>`;
}
